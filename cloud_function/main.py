    ...
    for user in config.target_users:
        ...
        for ev in resp.get("items", []) or []:
            if not _is_ooo_event(ev):
                continue
            # Note: We do NOT enforce organizer != owner here because we are scanning
            # existing OOO events on the calendar to count who is off.
            # If the OOO exists on the calendar, they are off, regardless of who put it there.
...
def _process_events(
    ...
    for event in events:
        ...
        # Requirement: Organizer must NOT be the owner of the calendar.
        organizer = event.get("organizer", {})
        organizer_email = organizer.get("email")

        # If organizer is the same as the calendar owner (user_email), skip it.
        # We also check organizer.self, which is True if the organizer matches the calendar owner.
        if organizer.get("self", False) or (organizer_email and organizer_email == user_email):
            LOGGER.info(
                "Skipping event %s because organizer (%s) is the calendar owner (%s)",
                event.get("id"),
                organizer_email,
                user_email,
            )
